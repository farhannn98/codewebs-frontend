...
// Interface untuk struktur data grafik bulanan
...
export default function AdminOrdersPage() {
  ...
  // 1. Cari fungsi handleSaveOrder bawaan kode lu yang lama
    ...
    autoTable(doc, {
      ...
      didParseCell: function (data) {
        if (data.section === "body" && data.column.index === 8) {
          ...
          if (text.includes("DP")) data.cell.styles.textColor = [37, 99, 235];
  ...
  // --- LOGIKAPEMETAAN DATA GRAFIK BULANAN SECARA DINAMIS ---
  const generateChartData = (): ChartDataPoint[] => {
    ...
    // Inisialisasi awal nilai omzet 0 rupiah di setiap bulan
    const monthlyMap: { [key: string]: number } = {};
    months.forEach((m) => {
      monthlyMap[m] = 0;
    });

    // Akumulasikan uang masuk riil berdasarkan bulan dari data API
    orders.forEach((o) => {
      ...
      if (monthlyMap[monthName] !== undefined) {
        monthlyMap[monthName] += fin.cashIn;
      }
    });

    return months.map((m) => ({
      name: m,
      Penjualan: monthlyMap[m],
    }));
  };

  const chartData = generateChartData();

  // --- RINGKASAN DATA KOTAK DASHBOARD ---
  ...
  return (
    <div className="space-y-8">
      ...
      {/* REVISI KOMPONEN GRAFIK (ANTI-POTONG & MENDUKUNG NOMINAL BESAR) */}
      <div className="bg-white p-6 sm:p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
        <div>
          <h3 className="font-black text-xl text-slate-900 tracking-tight">
            Grafik Kinerja Bulanan ({new Date().getFullYear()})
          </h3>
          <p className="text-xs text-slate-400 font-medium mt-0.5">
            Visualisasi fluktuasi omzet penjualan vs jumlah pesanan masuk.
          </p>
        </div>
            ...
            >
              ...
              <YAxis
                ...
                tickFormatter={(val: number) => {
                  if (val >= 1000000)
                    return `${(val / 1000000).toLocaleString("id-ID")} Jt`;
                  if (val >= 1000)
                    return `${(val / 1000).toLocaleString("id-ID")} Rb`;
                  return String(val);
                }}
              />
              <Tooltip
                        ...
                        <td className="px-4 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <button
                              ...
                              className="bg-slate-100 text-slate-600 p-1.5 rounded-lg"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-black text-slate-900">
              {editingOrder ? "Edit Rincian Pesanan" : "Tambah Pesanan Manual"}
            </h3>
            <form onSubmit={handleSaveOrder} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    className="border border-slate-200 text-slate-900 rounded-xl px-4 py-2.5 text-sm bg-white"
                  >
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-100">
                <button
                  ...
                  className="bg-slate-100 px-4 py-2.5 text-slate-900 rounded-xl text-xs font-bold uppercase"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold uppercase"
                >
                  Simpan
                </button>
              </div>
